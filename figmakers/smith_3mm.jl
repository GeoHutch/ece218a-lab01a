...
data = read_two_port("text_data/3mm.txt")
...
fig = SmithAxis([
                 PGFPlots.SmithData(Z.(S11_interp), style="blue", mark="none", legendentry="3mm Wire (Measured)"),
                 PGFPlots.SmithData([Zcross], style="cyan", mark="square", legendentry="Z=$(Z0*real(Zcross))"),
                 PGFPlots.SmithCircle(1+(1.659-1)/2,0.0,(1.659-1)/2; style="red", texlabel="3mm Wire (Theory)")
                 #  PGFPlots.SmithData(Z.(S11), style="red", mark="none", legendentry="5mm Wire (Theory)"),
])

save("figures/build/smith_3mm.pdf",fig)
